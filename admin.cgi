...
sub edituser {
  ...
  return include::errordiv('Error: User Not Found', qq[I cannot seem to find user ID <q>$uid</q> in the database.])
    if not ref $u;
  return userform($u, persistentvars => $persistentvars, hiddenpersist => $hiddenpersist);
}

sub newuser {
  return userform(+{}, persistentvars => $persistentvars, hiddenpersist => $hiddenpersist);
}
...
sub saveuser {
  ...
  $$u{username} =~ s/_+$//g;
  $$u{username} =~ s/^[^a-z]//gi;
  while (grep { $$_{id} ne $$user{id} } findrecord('users', 'username', $$u{username})) {
    if (not $$u{username} =~ /\d$/) { $$u{username} .= "0"; }
    $$u{username}++;
  }
  ...
  if (scalar @error) {
    return join "\n", @error, userform($u, persistentvars => $persistentvars, hiddenpersist => $hiddenpersist);
  } else {
